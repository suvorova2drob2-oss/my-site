/**
 * B2 Summer Intensive — step-by-step lesson wizard (shell only).
 */
(function () {
  var params = new URLSearchParams(window.location.search);
  var themeId = params.get("theme") || "";
  var theme = typeof B2_INTENSIVE_getTheme === "function" ? B2_INTENSIVE_getTheme(themeId) : null;
  var steps = window.B2_INTENSIVE_STEPS || [];
  ...
  document.title = "Lesson " + theme.num + " · " + theme.title + " · B2 Intensive";
  ...
  function updateNav() {
    ...
    if (elNext) {
      elNext.textContent = stepIndex === steps.length - 1 ? "Finish lesson" : "Next step →";
    }
    if (elProgress) {
