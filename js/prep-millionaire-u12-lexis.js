/**
 * Unit 12 Millionaire engine:
 * · Facts — reading comprehension (U12_MILLIONAIRE_FACTS)
 * · Gaps — cloze from plain texts using TARGET phrases in VOCAB_TTT_WORDS
 * · Paraphrase — definition-style MCQ (U12_MILLIONAIRE_PARAPHRASE_QUIZ)
 *
 * Depends on: unit12-ttt-word-bank.js, unit12-millionaire-passage-data.js
 */
(function (W) {
  "use strict";
  ...
  function filterParaphrase(topicIds) {
    var all = W.U12_MILLIONAIRE_PARAPHRASE_QUIZ || [];
    if (topicIds === null) return all.slice();
    if (!topicIds || !topicIds.length) return [];
    return all.filter(function (row) {
      return topicIds.indexOf(row.topicId) !== -1;
  ...
  function plainForTopic(topicId) {
    switch (topicId) {
      case "u12_reading_road_to_betterment":
        return String(W.U12_READING_ROAD_TO_BETTERMENT_LEXIS_PLAIN || "");
      case "u12_sports_idioms":
        return String(W.U12_SPORTS_IDIOMS_LEXIS_PLAIN || "");
      default:
        return "";
    }
  }

  /** Full passage for Facts · Text panel. */
  W.u12MillionaireFactFullPassage = function (topicId) {
   ...
   */
  W.buildU12MillionaireFacts = function (topicIds) {
   ...
   */
  W.buildU12MillionaireGapsFromPassages = function (topicIds) {
   ...
   */
  W.buildU12MillionaireParaphrase = function (topicIds) {
  ...
  /** @deprecated Use buildU12MillionaireParaphrase */
  W.buildU12MillionaireSynonyms = W.buildU12MillionaireParaphrase;

  /** Min questions to enable a tab */
  W.U12_MILLIONAIRE_MIN_TAB = 3;

  W.countU12MillionaireFacts = function (topicIds) {
    return filterFacts(topicIds).length;
  };

  W.countU12MillionaireParaphrase = function (topicIds) {
    return filterParaphrase(topicIds).length;
  };

  /** @deprecated Use countU12MillionaireParaphrase */
  W.countU12MillionaireSynonyms = W.countU12MillionaireParaphrase;

  W.countU12MillionaireGapsFeasible = function (topicIds) {
