/**
 * Prep hub — Level 8 linked folders (CPE Levels hub).
 */
(function (W) {
    "use strict";

    W.PREP_LEGACY_U8_VOCAB_FOLDER_ID = "prep_legacy_u8_vocab";

    function getU8VocabPack() {
        return W.PREP_HUB_U8_VOCAB_SEEDS || null;
    }
    ...
    function ensureLinkedFolder(opts) {
        ...
        var linkedUnit = opts.linkedUnit != null ? Number(opts.linkedUnit) : 8;
    ...
    W.ensurePrepUnit8VocabularyFolder = function () {
        var pack = getU8VocabPack();
        if (!pack) {
            try {
                console.warn("[Prep hub] Missing PREP_HUB_U8_VOCAB_SEEDS — load js/prep-hub-seeds-unit8-vocabulary.js");
            } catch (eSeed) {}
            return false;
        }
        return ensureLinkedFolder({
            folderId: W.PREP_LEGACY_U8_VOCAB_FOLDER_ID,
    ...
    W.ensurePrepUnit8LinkedShells = function () {
        W.ensurePrepUnit8VocabularyFolder();
    };

    W.prepHubUnit8EnsureByFolderId = function (folderId) {
        var fid = String(folderId == null ? "" : folderId);
        if (fid === String(W.PREP_LEGACY_U8_VOCAB_FOLDER_ID)) {
            W.ensurePrepUnit8VocabularyFolder();
            return true;
        }
        return false;
    ...
    W.hubIsUnit8LegacyLinkedFolderId = function (folderId) {
        return String(folderId == null ? "" : folderId) === String(W.PREP_LEGACY_U8_VOCAB_FOLDER_ID);
    };

    W.hubUnit8LegacyFolderHubHref = function (folderId) {
        if (String(folderId || "") === String(W.PREP_LEGACY_U8_VOCAB_FOLDER_ID)) {
            return (
                "unit8-vocabulary/cpe/index.html?course=cpe&backLabel=" + encodeURIComponent("Level 8")
            );
        }
        return null;
    };
