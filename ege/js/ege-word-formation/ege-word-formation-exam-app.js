/**
 * ЕГЭ Word Formation Exam (§25–29): словообразование в пропуске.
 */
(function () {
  var root = document.getElementById("ege-wf-app");
  if (!root) return;

  var pack = window.__EGE_WORD_FORMATION_EXAM__;
  ...
  function storageKey() {
    return "ege_wf_" + U.id + "_draft_v1";
  }

  function unitTimerSec() {
    var packD = window.__EGE_WORD_FORMATION_EXAM__ || {};
    var def =
  ...
  function refreshStatsBar() {
    ...
    if (!el || !U || !U.id) return;
    var br = window.__egeWordFormationExamStats;
    var main;
  ...
  function renderFeedbackCard(item, userRaw, ok) {
    ...
    html +=
      ...
      (ok ? "✓ Верно" : "✗ Ошибка") +
      "</span>";
    var tag = item.formationTag || item.grammarTag;
    if (tag) {
      html += '<span class="ege-gx-walk-tag">' + esc(tag) + "</span>";
    }
    html += "</div>";
  ...
  function renderFeedbackAfterSubmit(results) {
    ...
    var life =
      ...
      "<p><strong>Совет:</strong> смотри на слова вокруг пропуска — они подсказывают, нужно существительное, прилагательное, глагол или наречие.</p>";
  ...
  function renderUnitNavAfterCheck(okCount, total) {
    ...
    if (hasNext) {
      ...
      html +=
        '<a class="ege-gx-unit-nav-btn ege-gx-unit-nav-btn--hub" href="ege-word-formation.html"><span class="ege-gx-unit-nav-arrow" aria-hidden="true">←</span> К разделу Word Formation</a>';
    }
  ...
  function mountUnit() {
    ...
    var ui;
    var wfStats = window.__egeWordFormationExamStats;
    var wfPerfect = window.__egeExamUnitPerfectMark;
    for (ui = 0; ui < units.length; ui++) {
      html +=
        ...
        (wfPerfect && wfPerfect.optionPerfectClassAttr
          ? wfPerfect.optionPerfectClassAttr(wfStats, units[ui].id)
          : "") +
        ">" +
        esc(units[ui].title) +
        "</option>";
    ...
    if (U.examSection) {
      html +=
        '<p class="ege-gx-kicker">Word Formation · ' + esc(U.examSection) + "</p>";
    }
    html += '<h2 class="ege-gx-page-title">' + esc(U.title) + "</h2>";
    html +=
      '<p class="ege-gx-vibe"><strong>Как в бланке §25–29:</strong> из слова справа (MANAGE, STRESS…) нужно получить <strong>новое слово</strong> — с суффиксом или без: management, stressful. Пропуск слева, база в CAPS справа. <strong>Enter</strong> — следующий пропуск.</p>';
    ...
    html +=
      '<p class="ege-gx-route-note"><a href="ege-word-formation.html">&larr; Другие задания Word Formation</a></p>';
    html += "</div>";
    ...
    document.getElementById("ege-gx-check").addEventListener("click", function () {
      ...
      var wfSel = document.getElementById("ege-gx-unit-select");
      if (
        wfSel &&
