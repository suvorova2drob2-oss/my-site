/**
 * Speaking · question-master (Task 2) → Statistics Hub + activity log.
 */
(function (w) {
  var KEY = "ege_speaking_question_master_scores";
  var GYM_KEY = "question-master";
  var SKILL = "Speaking: question-master";
  ...
  function recordAttempt(unitId, percent) {
    ...
    if (w.__egeActivityTracker && typeof w.__egeActivityTracker.recordActivity === "function") {
      w.__egeActivityTracker.recordActivity({
        label: SKILL,
        href: "ege-speaking-question-master.html",
        score: p
      });
    }
    return prev;
  ...
  function mergeIntoExamStatisticsState(state, clampScoreFn) {
    ...
    if (agg == null) return;
    if (!Object.prototype.hasOwnProperty.call(state.gym, GYM_KEY)) {
      state.gym[GYM_KEY] = 0;
    }
    state.gym[GYM_KEY] = clamp(Math.max(state.gym[GYM_KEY] || 0, agg));
  }

  w.__egeSpeakingQuestionMasterStats = {
    KEY: KEY,
    GYM_KEY: GYM_KEY,
