/**
 * Scroll & Speak — teen session wizard (shell only).
 */
(function () {
  var params = new URLSearchParams(window.location.search);
  var themeId = params.get("theme") || "";
  var theme = typeof SCROLL_SPEAK_getTheme === "function" ? SCROLL_SPEAK_getTheme(themeId) : null;
  var steps = window.SCROLL_SPEAK_STEPS || [];
  ...
  if (!theme || !elRoot) {
    if (elRoot) {
      elRoot.innerHTML =
        '<div class="int-error"><p>Theme not found. <a class="int-back" href="index.html">Back to club</a></p></div>';
    }
    return;
  }

  document.title = theme.title + " · Scroll & Speak";
  ...
  var headMeta = document.getElementById("lesson-meta");
  ...
  if (headMeta) {
    var parts = [];
    if (theme.lenses && theme.lenses.length) {
      parts.push("<strong>Talk about:</strong> " + theme.lenses.map(escapeHtml).join(" · "));
    }
    if (theme.challenge) {
      parts.push("<strong>Challenge:</strong> " + escapeHtml(theme.challenge));
    }
    headMeta.innerHTML = parts.join("<br />");
  }
  ...
  function renderStage() {
    ...
    var teacherExtra = "";
    if (step.id === "trend" && theme.searchHints) {
      teacherExtra =
        '<p class="ss-search-hint"><strong>Search ideas:</strong> ' +
        escapeHtml(theme.searchHints) +
        "</p>";
    }
